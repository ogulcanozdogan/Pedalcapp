...
import 'PassengerDetailHourly.dart';

class RideInformationHourly extends StatefulWidget {
  const RideInformationHourly({super.key});
  @override
  State<RideInformationHourly> createState() => _RideInformationState();
}

class _RideInformationState extends State<RideInformationHourly> {
  ...
  String _selectedDuration = '30 Minutes'; // default duration
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      final rideDuration = _getDurationInMinutes(_selectedDuration);
      ...
      var booking = Provider.of<BookingHourly>(context, listen: false);
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => PassengerDetailHourly()),
      );
    }
  }

  double _getDurationInMinutes(String duration) {
    switch (duration) {
      case '30 Minutes':
        return 30;
      case '1 Hour':
              ...
              children: [
                ListView(
                  children: [
                    ...
                    const Text("Duration of Service"),
                    const SizedBox(height: 5),
                    DropdownButtonFormField(
                    ...
                    _buildPlaceAutoCompleteTextField(
                        _pickupController, _pickupFocusNode, "Please, enter start address."),
                    ...
                    const SizedBox(height: 5),
                    _buildServiceDetailsTextField(_serviceDetailsController, _serviceDetailsFocusNode, "Please, send us more details about this service."),
                    const SizedBox(height: 15),
  ...
  String _formatAddress(List<webservice_places.AddressComponent> addressComponents, String formattedAddress, String customPlaceName) {
    ...
    if (customPlaceName.isNotEmpty && !addressWithoutCountry.contains(customPlaceName)) {
      return '$addressWithoutCountry ($customPlaceName)';
    }
    return addressWithoutCountry;
  }

  bool _isValidPostalCode(List<webservice_places.AddressComponent> addressComponents) {
    for (var component in addressComponents) {
      if (component.types.contains('postal_code') && _allowedPostalCodes.contains(component.longName)) {
        return true;
      }
    }
    return false;
